/*
 * chelly-exec.c — Non-interactive command execution via fork+exec+pipe.
 *
 * Uses plain pipes to capture stdout/stderr and return them as strings.
 * Used for programmatic command execution (AI dispatch, file ops, diagnostics).
 *
 * JNI class: dev.chelly.execbridge.ChellyJNI
 */
...
Java_dev_chelly_execbridge_ChellyJNI_execSubprocess(
        JNIEnv *env,
        ...
        jint    timeoutMs)
{
    ...
    const char *cwd        = (*env)->GetStringUTFChars(env, cwdJ,        NULL);
    const char *command    = (*env)->GetStringUTFChars(env, commandJ,    NULL);

    if (!linkerPath || !bashPath || !ldLibPath || !homePath || !cwd || !command) {
    ...
    if (pid == 0) {
        ...
        /* Use cwd if non-empty, otherwise fall back to homePath */
        const char *workDir = (cwd[0] != '\0') ? cwd : homePath;
        if (chdir(workDir) != 0) { /* non-fatal */ }
    ...
    /* Create Java strings from buffers — realloc to ensure space for null terminator */
    if (stdout_len >= stdout_cap) {
        stdout_buf = (char *)realloc(stdout_buf, stdout_len + 1);
    }
    if (stderr_len >= stderr_cap) {
        stderr_buf = (char *)realloc(stderr_buf, stderr_len + 1);
    }
    ...
    (*env)->ReleaseStringUTFChars(env, cwdJ,         cwd);
...
release_strings:
    ...
    if (cwd)        (*env)->ReleaseStringUTFChars(env, cwdJ,        cwd);
